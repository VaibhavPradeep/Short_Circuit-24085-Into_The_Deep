...
@Config
@TeleOp(name = "use this teleop (with PID)")
public class TeleOp18WithSorterPID extends OpMode {
...
//        double pidOutput = controller.calculate(currentPos, sorterTarget);
//
...
//
//        // Increase target position when A is pressed
//        if (gamepad2.a && !prevPressed) {
//            sorterTarget += encoderAmount;
//        }
//
//        prevPressed = gamepad2.a;
...
//        prevPressed = aPressed;
        ...
        if (aPressed && !prevPressed) {
            ...
            pos += encoderAmount;
            double pidOutput = controller.calculate(sorterMotor.getCurrentPosition(), pos);
            sorterMotor.setPower(pidOutput);
        } else {
            int pos = sorterMotor.getCurrentPosition();
            double pidOutput = controller.calculate(sorterMotor.getCurrentPosition(), pos);
